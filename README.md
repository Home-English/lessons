...
> В английском языке любое/простое предложение должно иметь:
  ...
  ~ ( только: 1-у идею; 1-о время и место действия только; 1-ин тип-предложения (шаблон: состояние|поведение|обстоятельство|вопрос); )


-------------------------------------[ 1-категория ]
В английском языке, само по себе 1-но слово - не имеет никакого значения.
Но существую специальные связки из нескольких слов, которые внутри предложения могут уже что-то обозначать.
1. (обязательные) Слова указывающие на время действия:
   ~ <going to, ing, ed> и <will, was, were, am, is, are>
2. (характеризирующие предмет) Дополнительные слова:
   ~ (Физический) неодушевленный предмет ... A / The
...
-------------------------------------[ 2-категория ]
Правила построения простого предложния в английском языке:
> В каждом предложении должна быть заложенная основная идея... и на нее будет указывать только одно-единственное слово (в котором и будет заложена основная идея для всего предложения).
  ...
  3. Уточняют обстоятельство.
     Может использоваться только как дополненние в сочетании либо с выражением внутреннего состояния или описанием поведения (самостоятельно НЕиспользуется)..
  ...
  4. Чтобы спросить о чем либо.
     Может использоваться для всех случаев...
     ~ в таком предложении вопросительное слово, которое выражает идею - является (СПЕЦИАЛЬНЫМ-ДОПОЛНЕНИЕМ) и меняет порядок слов...
> Шаги для построения простого предложния в английском языке:
  ...
  5. Уточняющие слова не являются обизательными в английском предложении и их можно по желанию указывать
...
> Правила для построения сложного предложния в английском языке:
  ...
  2. Определить тип(ы) для всех этих слов (выражающих идеи: состояние|поведение|обстоятельство|вопрос)
