...
```
 ...
 [1] PRESENT SIMPLE                  + +    [3] PRESENT CONTINUOS           | > am/are/is +ing
    (настоящее:                      + +       (настоящеен время:           | > место нахождения (пребывания)
...
```
-------------------------------------[ Правила (шаги) построения слов в английском языке ]
1. В английском языке любое предложение (даже самое простое) всегда должно описывать какое-нибудь действие (даже если ничего не происходит..)
   ...
   'IT IS THING' <ОНО БЫТЬ ВЕЩЬ>
   > нужно представить себе такой шаблон...
2. В каждом предложении должна быть заложенная основная идея... такая основная идея всегда будет одной единственной в предложении
   - и на нее будет указывать только одно единственное слово (в котором и будет заложена основная идея для всего предложения).
   В английском языке такое единственное слово, которое выражает идею всего предложения всегода ставится в конце предложения
   - и такое слово, которое выражает идею - заменяет 'ВЕЩЬ'
   > нужно представить себе основную идею которую нужно выразить...
   > <ОНО БЫТЬ идея>
3. В любом предложении есть ПIДМЕТ и ПРИСУДОК (иннициатор и тип действия).
   > нужно представить себе иннициатор и тип действия - заменяет (иннициатор) 'ОНО' и (тип действия) 'БЫТЬ'...
   > <иннициатор тип-действия идея>
4. В английском языке предложение всегда должно указывать на время когда происходит это действие.
   > нужно представить себе в каком периоде времени происходит действие - к глаголу 'БЫТЬ' добавляется либо 'ED', либо 'ING', либо 'GOING TO' (или заменяется WAS/DAS/WILL)...
   > <иннициатор GOING TO+тип-действия+ING/ED идея>
   > <иннициатор WAS/DAS/WILL идея>
5. Помимо этого, в предложении могут встречаться и элементы которые уточняют - это слова которые дополнительно могут указывать на МЕСТО и ВРЕМЯ.
   > Уточняющие слова не являются обизательными в английском предложении и их можно по желанию либо использовать либо не использовать...
   > <МЕСТО-ВРЕМЯ + иннициатор GOING TO+тип-действия+ING/ED идея>
   > <иннициатор GOING TO+тип-действия+ING/ED идея + МЕСТО-ВРЕМЯ>

-------------------------------------[ Типы предложений бывают ]
...
3. Дополнение.
   Может использоваться только в сочетании либо с выражением внутреннего состояния или описанием поведения (самостоятельно НЕиспользуется)..
   > в таком предложении слово, которое выражает идею - является ПРИЛАГАТЕЛЬНЫМ (которое дополняет либо существительное либо прилагательное)...

-------------------------------------[ Распределяем смысловую нагрузку предложения ]
...
```
   ...
   (распределяем смысловую нагрузку предложения в английском языке)
   Если, в английском языке, предложение неполное где нехватает либо ПОДЛЕЖАЩЕГО или СКАЗУЕМОГО или ДОПОЛНЕНИЯ
